...
export default function Dashboard() {
  ...
  const { athlete, fitness, recentActivities } = mockStats
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 mt-1">Tu estado actual · {weeksToRace} semanas para el Ironman 70.3 Málaga</p>
          </div>
          {!connected ? (
            <button
              onClick={() => setConnected(true)}
              className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-white transition-all hover:opacity-90"
              style={{ background: 'linear-gradient(135deg, #fc4c02, #e63900)' }}
            >
              <span>🔗</span> Conectar Strava
            </button>
          ) : (
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl" style={{ background: '#1a2e1a', border: '1px solid #166534' }}>
              <div className="w-2 h-2 rounded-full bg-green-400"></div>
              <span className="text-green-400 text-sm font-medium">Strava conectado</span>
            </div>
          )}
        </div>

        {!connected && (
          <div className="rounded-2xl p-8 text-center" style={{ background: '#111827', border: '1px solid #fc4c02' }}>
            <div className="text-5xl mb-4">🔗</div>
            <h2 className="text-xl font-bold text-white mb-2">Conecta tu Strava para análisis personalizado</h2>
            <p className="text-gray-400 mb-6">Analizaré tus últimas 90 días de actividad para calibrar tu plan de entrenamiento</p>
            <button
              onClick={() => setConnected(true)}
            ...
            <p className="text-gray-600 text-xs mt-3">
              Necesitas configurar STRAVA_CLIENT_ID y STRAVA_CLIENT_SECRET en .env.local
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl p-6" style={{ background: '#111827', border: '1px solid #1f2937' }}>
            <h2 className="text-white font-bold text-lg mb-4">Actividades recientes</h2>
              ...
              {recentActivities.map(act => (
                <div key={act.id} className="flex items-center justify-between p-3 rounded-xl" style={{ background: '#1f2937' }}>
                  ...
                  {act.average_heartrate && (
                    <div className="text-red-400 text-sm font-medium">{act.average_heartrate} bpm</div>
                  )}
                </div>
              ))}
            </div>
